<?php
	...
	if(!$adminDetails['status']) {
		...
		if(!isset($_POST['restriction_name'])) {
			echo json_encode(array('status' => false, 'message' => 'Invalid request'));
		}
		else {
			$status = $admin->addRestriction($_POST['restriction_name'], $conn);
			if($status) {
				echo json_encode(array('status' => true, 'message' => 'Restriction added'));
			}
			else {
				echo json_encode(array('status' => false, 'message' => 'Unable to add restriction'));
			}
		}
	}