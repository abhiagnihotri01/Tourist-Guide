<?php
	...
	if(!$adminDetails['status']) {
		...
		if(!isset($_POST['id'])) {
			...
			$status = $admin->deleteFeedback($_POST['id'], $conn);
			if($status) {
				echo json_encode(array('status' => true, 'message' => 'Feedback deleted'));
			}
			else {
				echo json_encode(array('status' => false, 'message' => 'Unable to delete feedback'));
			}
		}
	}