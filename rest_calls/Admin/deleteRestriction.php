<?php
	...
	if(!$adminDetails['status']) {
		...
		if(!isset($_POST['id'])) {
			...
			$status = $admin->deleteRestriction($_POST['id'], $conn);
			if($status) {
				echo json_encode(array('status' => true, 'message' => 'Restriction deleted'));
			}
			else {
				echo json_encode(array('status' => false, 'message' => 'Unable to delete restriction'));
			}
		}
	}